...
**Status**: `[x]` **COMPLETE / ARCHIVED (2026-06-15)** — Phase 0 + Phase 1 (1.1–1.5) + Phase 2 (2.1–2.8) all byte-identical; Phase 3 B1 (registry, byte-identical, ADR-0007) + B2 (structured render, the one reviewed output change, ADR-0008) landed; B4 dropped as mis-scoped; B3 NO-GO. Promoted to `main` via merge commit. See changelog Phase 1/2/3 entries.
**Last updated**: 2026-06-15 — **Phase 3 closed.** B2 cutover landed (structured render replaces regex-on-rendered-HTML, F7 full fix; `RENDER_V2` default ON, flag + 4 regex helpers + dead `timelineHomeView` deleted; reviewed diff = `index.html` + `reviews/index.html`, no URL changed, ADR-0008). B1 complete (roster registry, byte-identical, ADR-0007). B4 dropped (image regex is a legitimate cross-source semantic transform, not the B2 anti-pattern — see memex `pattern-regex-on-html-not-always-antipattern`).
