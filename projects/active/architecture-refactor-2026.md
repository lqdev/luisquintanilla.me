...
**Last updated**: 2026-06-15 — **Phase 3 B2 complete (cutover landed)**: structured render product replaces regex-on-rendered-HTML (F7 full fix), the one output-changing bet. `RENDER_V2` default ON; flag + 4 regex helpers + dead `timelineHomeView` deleted; reviewed diff = `index.html` + `reviews/index.html` (both characterized, 1 strict bug-fix), no URL changed, ADR-0008. **Phase 3 B1 complete** (content-type roster registry, byte-identical, ADR-0007). B4 pending go/no-go.
