...
by `embed.record`. Linked Markdown image previews are omitted from link-post excerpts because the
external card already carries the target media title and preview; standalone images remain in the
excerpt as meaningful response content. The site page still renders the original Markdown unchanged.
Native reposts carry only the resolved `{uri, cid}` subject.
...
| `test-scripts/test-atproto-response-mapping.fsx` | 50 response target, routing, text, facets, tags, and record-shape assertions |
