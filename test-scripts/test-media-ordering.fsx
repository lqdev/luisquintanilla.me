...
// Expected HTML format: <li><a href="/media/...">Title</a> • MMM dd, yyyy</li>
...
// Function to parse date string to DateTimeOffset for comparison (matches Builder.fs implementation)
let parseDate (dateStr: string) =
    DateTimeOffset.ParseExact(dateStr, "MMM dd, yyyy", System.Globalization.CultureInfo.InvariantCulture)
...
// Parse dates once for efficient comparison
let parsedDates = dates |> List.map parseDate
let expectedOrder = parsedDates |> List.sortByDescending id

if parsedDates = expectedOrder then
